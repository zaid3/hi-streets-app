...
export default function JobApplicationsPanel() {
  ...
  return <div className="privacy-card"><h2><Briefcase size={20} /> Job applications</h2><p className="muted">Applications from people who applied in HiStreets. CV links are private and temporary. Contact applicants directly by email, phone or WhatsApp.</p>{status && <p className="form-status">{status}</p>}{items.map(item => <article className="post-card" key={item.id}><div><Briefcase size={20} /></div><div><h3>{item.job_title}</h3><p><strong>{item.applicant_name}</strong> applied to {item.business_name}</p><div className="post-meta"><span><Mail size={14} /> {item.applicant_email}</span><span><Phone size={14} /> {item.applicant_phone}</span></div>{item.cover_note && <p>{item.cover_note}</p>}<div className="applicant-actions"><a href={`mailto:${item.applicant_email}?subject=${encodeURIComponent(`Your application for ${item.job_title}`)}`}><Mail size={16} /> Email result</a><a href={`tel:${item.applicant_phone}`}><Phone size={16} /> Call</a><a href={whatsappUrl(item.applicant_phone)} target="_blank" rel="noreferrer"><MessageCircle size={16} /> WhatsApp</a>{item.cv_url ? <a href={item.cv_url} target="_blank" rel="noreferrer"><FileText size={16} /> Open CV</a> : <span className="muted"><FileText size={16} /> CV unavailable</span>}</div></div></article>)}</div>
}
