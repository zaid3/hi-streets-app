...
import{inNewhamBounds}from'./newhamSeedData'
...
export async function saveBusinessProfile(userId,profile,currentId){
  ...
  if(!inNewhamBounds({lat:payload.lat,lng:payload.lng}))return{success:false,error:'This app is Newham-only right now. Use a Newham business location.'}
  try{
    if(currentId){
...
export function businessReadyForOffers(business){
  ...
  if(!inNewhamBounds({lat:Number(business.lat),lng:Number(business.lng)}))return{ready:false,reason:'Only Newham business locations can publish offers right now.'}
