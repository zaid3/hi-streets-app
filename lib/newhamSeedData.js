...
function normaliseBounds(bounds){
  return bounds&&typeof bounds==='object'&&Number.isFinite(Number(bounds.south))?bounds:NEWHAM_BOUNDS
}

export function inNewhamBounds(point,bounds=NEWHAM_BOUNDS){
  if(!point)return false
  const b=normaliseBounds(bounds)
  const lat=Number(point.lat),lng=Number(point.lng)
  return Number.isFinite(lat)&&Number.isFinite(lng)&&lat>=b.south&&lat<=b.north&&lng>=b.west&&lng<=b.east
}
...
export function boundsIntersect(a,b=NEWHAM_BOUNDS){
  const bounds=normaliseBounds(b)
  if(!a)return true
  return !(a.west>bounds.east||a.east<bounds.west||a.south>bounds.north||a.north<bounds.south)
}
