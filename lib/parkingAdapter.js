...
// Priority:
// 1) council open parking bay data where available
// 2) verified Newham seed data for the MVP area
// 3) OSM/Overpass where parking tags exist
// 4) no fake street bays outside reliable data coverage
...
export async function getParkingData(bounds){
  ...
  const[council,osm]=await Promise.all([fetchCouncilParking(bounds),fetchOverpassSafe(bounds)])
  return mergeById([...council,...seed,...osm])
}
...
function priority(item){return item.source==='council'?0:item.source==='verified_seed'?1:item.source==='osm'?2:3}
