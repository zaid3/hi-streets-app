...
const LEGEND=[['blue','#8E44AD','Verified Blue Badge bay'],['cpz','#078d16','CPZ background only']]
...
function Header({onSearch,blueBadgeCount,lineCount}){return <div className="newham-top"><button onClick={onSearch} className="newham-search"><span>♿</span><b>Search Newham Blue Badge parking</b></button><div className="newham-status"><b>{blueBadgeCount}</b> Blue Badge bays · <b>{lineCount}</b> verified bay lines</div></div>}
...
function EmptyBlueBadgeNote({count}){if(count)return null;return <div className="blue-empty-note"><b>No verified Blue Badge bays in this view yet.</b><br/>Add surveyed bays or import official disabled-bay data. The map will not show guessed parking.</div>}
export default function MapPage(){
  ...
  return <div className="newham-map-shell blue-badge-mode"><style jsx global>{`
    ...
    .newham-search{pointer-events:all;width:min(460px,calc(100vw - 28px));height:56px;border:1px solid #e5e1ef;background:#fff;border-radius:14px;box-shadow:0 8px 24px rgba(25,20,55,.12);display:flex;align-items:center;gap:12px;padding:0 15px;color:#0b0628;font-size:16px;cursor:pointer;text-align:left}.newham-search span{font-size:24px;color:#8E44AD}.newham-search b{font-weight:900;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
    .newham-status{pointer-events:all;background:#0b0628;color:#fff;border-radius:999px;padding:7px 12px;font-size:12px;font-weight:800;box-shadow:0 6px 18px rgba(11,6,40,.22);max-width:calc(100vw - 24px);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
    .newham-legend{position:absolute;left:50%;bottom:max(16px,env(safe-area-inset-bottom));transform:translateX(-50%);z-index:300;width:min(480px,calc(100vw - 24px));display:flex;gap:7px;overflow-x:auto;padding:8px;background:rgba(255,255,255,.94);border:1px solid #e7e3f0;border-radius:16px;box-shadow:0 12px 34px rgba(25,20,55,.16);scrollbar-width:none}.newham-legend::-webkit-scrollbar{display:none}.legend-pill{flex:0 0 auto;display:flex;align-items:center;gap:7px;background:#f7f6fc;border-radius:999px;padding:8px 10px;font-size:12px;font-weight:900;color:#3d394f}.legend-pill span{width:14px;height:14px;border-radius:4px;display:block}
    .blue-empty-note{position:absolute;left:50%;bottom:82px;transform:translateX(-50%);z-index:290;width:min(460px,calc(100vw - 24px));background:#fff;border:1px solid #e7e3f0;border-radius:16px;box-shadow:0 12px 34px rgba(25,20,55,.16);padding:13px 15px;color:#3d265f;font-size:13px;font-weight:800;line-height:1.35}
