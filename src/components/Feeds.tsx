...
type SortMode = 'newest' | 'nearest' | 'ending' | 'easy'
...
export function Feed({ type, posts }: FeedProps) {
  ...
  const items = useMemo(() => {
    ...
    if (sortMode === 'easy' && type === 'job') filtered = filtered.filter(p => p.type === 'job')
    return filtered
  ...
  return (
    <section className="feed-screen">
      ...
      <div className="chip-row">
        ...
        {type === 'job' && <button type="button" onClick={() => setSortMode('easy')} className={sortMode === 'easy' ? 'active' : ''}>Easy apply</button>}
      </div>
