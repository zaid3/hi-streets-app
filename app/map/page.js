...
import{NEWHAM_CENTER,NEWHAM_BOUNDS,boundsAround,boundsIntersect,clampToNewham,inNewhamBounds,newhamPlaces}from'../../lib/newhamSeedData'
...
function newhamBoundsAround(center){const b=boundsAround(clampToNewham(center),0.02);return{south:Math.max(b.south,NEWHAM_BOUNDS.south),west:Math.max(b.west,NEWHAM_BOUNDS.west),north:Math.min(b.north,NEWHAM_BOUNDS.north),east:Math.min(b.east,NEWHAM_BOUNDS.east)}}
...
function DataBar({segments,offers,googleParking}){
  ...
  const label=official?`${official} Newham road lines`:googleParking.length?`${googleParking.length} Newham parking places`:total?`${total} Newham parking records`:'No Newham parking found here yet'
  return <div className={`data-bar ${total?'':'empty'}`}><span>{label}</span>{!official&&googleParking.length>0&&<span>car parks first</span>}{official>0&&googleParking.length>0&&<span>{googleParking.length} car parks</span>}{offers.length>0&&<span>{offers.length} local offers</span>}</div>
}
function ActionSheet({collapsed,setCollapsed,onPlan,onNearby,onBusiness,onOffers,offers,parkingCount,official,googleParking}){
  if(collapsed)return <div className="action-sheet compact"><div className="sheet-handle"/><button onClick={()=>setCollapsed(false)} className="action-compact-btn">Newham parking and offers</button></div>
  const summary=official?`${official} official Newham road records nearby`:googleParking?`${googleParking} Newham parking places from Google`:'Search a Newham road or postcode'
  return <div className="action-sheet"><div className="sheet-handle"/><div className="action-title">Newham parking</div><div className="action-summary"><b>{parkingCount}</b> results. {summary}</div><button onClick={onPlan} className="action-row"><span className="action-circle">⌕</span><span>Search Newham road or postcode</span><span className="chevron">›</span></button><button onClick={onNearby} className="action-row"><span className="action-circle">P</span><span>Parking filters</span><span className="chevron">›</span></button>{offers>0&&<button onClick={onOffers} className="action-row"><span className="action-circle">%</span><span>Local business offers</span><span style={{marginLeft:'auto',fontSize:14,fontWeight:900,color:'#ff681f'}}>{offers} live</span><span className="chevron">›</span></button>}<button onClick={onBusiness} className="action-row"><span className="action-circle">+</span><span>Add free Newham business offer</span><span className="chevron">›</span></button><button onClick={()=>setCollapsed(true)} className="plain-collapse">Hide</button></div>
}
export default function MapPage(){
  ...
  useEffect(()=>{navigator.geolocation?.getCurrentPosition(p=>{const loc={lat:p.coords.latitude,lng:p.coords.longitude};setCenter(clampToNewham(loc));if(inNewhamBounds(loc))setZoom(17)},()=>setCenter(UK),{enableHighAccuracy:true,timeout:9000,maximumAge:10000})},[])
  const handleBoundsChange=useCallback(async(bounds)=>{clearTimeout(loadTimer.current);loadTimer.current=setTimeout(async()=>{try{if(!boundsIntersect(bounds)){setSegments([]);setPlaces(newhamPlaces);return}const scoped={south:Math.max(bounds.south,NEWHAM_BOUNDS.south),west:Math.max(bounds.west,NEWHAM_BOUNDS.west),north:Math.min(bounds.north,NEWHAM_BOUNDS.north),east:Math.min(bounds.east,NEWHAM_BOUNDS.east)};const[parking,poi]=await Promise.all([getParkingData(scoped),getPlacesData(scoped)]);setSegments(parking);setPlaces(poi.length?poi:newhamPlaces)}catch(e){console.error(e)}},450)},[])
  useEffect(()=>{handleBoundsChange(newhamBoundsAround(UK));getLiveOffers().then(setOffers);const unsub=subscribeToOffers(setOffers);return unsub},[handleBoundsChange])
  const selectSeg=useCallback((seg)=>{const lat=seg.lat||seg.coords?.[0]?.[0],lng=seg.lng||seg.coords?.[0]?.[1];if(lat&&lng){setCenter(clampToNewham({lat,lng}));setZoom(17)}setSelectedOffer(null);setSelectedSeg(seg);setActionCollapsed(true)},[])
