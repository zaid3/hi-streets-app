...
export function parseWhatsAppOffer(message){
  if(!message?.trim())return{valid:false,error:'Empty message'}
  const t=message.trim()
  if(!t.toUpperCase().startsWith('#OFFER'))return{valid:false,error:'Message must start with #OFFER',example:'#OFFER 20% off lunch today until 5pm'}
  const body=t.replace(/^#OFFER\s*/i,'').trim()
  if(body.length<5)return{valid:false,error:'Offer text too short'}
  const um=body.match(/until\s+(\d{1,2}(?::\d{2})?\s*(?:am|pm)?)/i)
  const today=/\btoday\b/i.test(body)
  const expiresAt=um?parseTime(um[1]):today?eod():null
  return{valid:true,title:body,shortLabel:body.slice(0,22),description:body,expiresAt,source:'whatsapp'}
}
function parseTime(s){const now=new Date(),m=s.match(/(\d{1,2})(?::(\d{2}))?\s*(am|pm)?/i);if(!m)return eod();let h=parseInt(m[1]),mi=m[2]?parseInt(m[2]):0,ap=m[3]?.toLowerCase();if(ap==='pm'&&h!==12)h+=12;if(ap==='am'&&h===12)h=0;now.setHours(h,mi,0,0);if(now<new Date())now.setDate(now.getDate()+1);return now.toISOString()}
