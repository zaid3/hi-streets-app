...
import{newhamParkingSegments}from'./newhamSeedData'
...
export async function getParkingData(bounds){
  ...
  const seed=fetchSeedBlueBadge(bounds)
  return mergeById([...verified,...surveyed,...seed])
}
...
function fetchSeedBlueBadge(bounds){
  return newhamParkingSegments
    .filter(seg=>seg.type==='disabled')
    .filter(seg=>!bounds||seg.coords?.some(([lat,lng])=>lat>=bounds.south&&lat<=bounds.north&&lng>=bounds.west&&lng<=bounds.east))
    .map(seg=>({
      ...seg,
      color:'#8E44AD',
      lat:seg.lat||seg.coords?.[0]?.[0],
      lng:seg.lng||seg.coords?.[0]?.[1],
      source:'seed_blue_badge',
      sourceName:'Seed Blue Badge bay',
      confidence:'low',
      dataNote:'Seed example Blue Badge bay. Replace with official/surveyed data before public launch.'
    }))
}
...
function priority(item){return item.source==='dtro'||item.source==='verified_blue_badge'?0:item.source==='surveyed_blue_badge'?1:item.source==='seed_blue_badge'?2:3}
