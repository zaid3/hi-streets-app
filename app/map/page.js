...
function DataBar({segments,offers}){
  ...
  const label=segments.length?`${segments.length} parking here`:'No verified road data here'
  return <div className={`data-bar ${segments.length?'':'empty'}`}><span>{label}</span><span>{official} official</span><span>{offers.length} offers</span></div>
}
function ActionSheet({collapsed,setCollapsed,onPlan,onNearby,onBusiness,onOffers,offers,parkingCount,official}){
