...
export default function Profile({ onPost }: Props) {
  ...
  if (!signedIn) return <section className="profile-screen"><div className="auth-card"><ShieldCheck size={34} /><h1>Business access</h1><p>Normal users do not need login to browse, find offers, or apply for jobs. Businesses use this area to register or claim a business, post offers/jobs/free meals, and view applications. Super Admin also logs in here.</p><input type="email" placeholder="you@example.com" value={email} onChange={e => setEmail(e.target.value)} /><div className="auth-actions"><button onClick={() => sendMagicLink('signup')}><UserPlus size={18} /> Sign up / register or claim</button><button onClick={() => sendMagicLink('login')}><LogIn size={18} /> Login existing account</button></div>{message && <p className="form-status">{message}</p>}<p className="tiny-links"><a href="/privacy.html">Privacy</a> · <a href="/terms.html">Terms</a></p></div></section>
