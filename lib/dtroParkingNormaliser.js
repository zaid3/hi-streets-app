...
function classifyProvision(provision){
  const types=regulationTypes(provision).join(' ').toLowerCase()
  const description=String(provision.provisionDescription||'').toLowerCase()
  const text=`${types} ${description}`
  ...
  if(/no waiting|no parking|no stopping|red route|prohibition|kerbsidenowaiting|kerbsidenostopping/.test(text))return{type:'no_parking',color:'#9d9da5'}
  if(/no loading|kerbsidenoloading|loading/.test(text))return{type:'loading',color:'#ff681f'}
  if(/permit|resident/.test(text))return{type:'resident',color:'#078d16'}
  if(/limited|paid|pay|meter|parking place|parking bay|kerbsidelimitedwaiting|kerbsideparkingplace/.test(text))return{type:'paid',color:'#0b73d9'}
...
function numericLengthMetres(coords){
...
function lengthMetres(coords){
  const length=numericLengthMetres(coords)
...
function usableDtroLine({coords,name,restriction,length}){
  if(!Array.isArray(coords)||coords.length<2)return false
  if(badText(name)||badText(restriction))return false
...
export function normaliseDtroParking(dtro){
  ...
  const troName=firstText(source.troName,dtro?.troName)
  ...
  return provisions.flatMap((provision,index)=>{
    ...
    const name=firstText(place?.description,troName)
    const restriction=firstText(regs,description)
    if(!usableDtroLine({coords,name,restriction,length}))return[]
    return[{
      ...
      council,
      confidence:'high',
      dataNote:'Official DfT D-TRO line geometry. Always check the roadside sign before parking.',
    }]
  })
}
