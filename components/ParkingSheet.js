...
function NavigateModal({segment,onClose}){
  ...
  function open(app){
    ...
    const urls={apple:`maps://maps.apple.com/?daddr=${p.lat},${p.lng}`,google:`https://maps.google.com/?q=${p.lat},${p.lng}`,waze:`https://waze.com/ul?ll=${p.lat},${p.lng}&navigate=yes`}
  ...
  return(
    <div className="modal-dim">
      <div className="modal-card">
        ...
        <div className="modal-title">Navigate to selected parking</div>
        <div className="nav-illustration"><div className="nav-route"/><div className="nav-badge">Parking</div><div className="nav-car">🚕</div></div>
        <p className="modal-copy">Choose your navigation app and get directions to this parking place.</p>
...
export default function ParkingSheet({segment,onClose,destination}){
  ...
  const st=getStatus(segment),no=isNoParking(segment.type),src=sourceInfo(segment)
  return(
    <>
        ...
        <div className="card-pad">
          <div style={{display:'flex',alignItems:'flex-start',gap:18,justifyContent:'space-between'}}>
            <div style={{display:'flex',gap:18,alignItems:'center',minWidth:0}}>
              <div className="status-tile" style={{background:st.color,fontSize:st.icon.length>1?22:38}}>{st.icon}</div>
              <div style={{minWidth:0}}>
                ...
                <div style={{display:'flex',gap:8,flexWrap:'wrap',marginTop:10}}>
                  <span className="info-chip" style={{background:src.tone,color:src.color,fontSize:13}}>{src.trust}</span>
                  <span className="info-chip" style={{background:'#f7f6fc',fontSize:13}}>{src.label}</span>
                </div>
              </div>
            </div>
          ...
          <div className="timeline-card">
            <div className="timeline-title">
              ...
              <div className="segment-switch"><button className={!week?'active':''} onClick={()=>setWeek(false)}>Day</button><button className={week?'active':''} onClick={()=>setWeek(true)}>Week</button></div>
            </div>
          ...
          <div style={{padding:'28px 0',borderBottom:'1px solid #eeeaf7'}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'20px 1fr auto',gap:'0 16px',alignItems:'start'}}>
              ...
              <div><div style={{fontSize:18,color:'#77768a'}}>Selected parking</div><div style={{fontSize:20,fontWeight:900}}>{segment.name||st.sub}</div></div>
              {walk&&<div style={{fontSize:20,fontWeight:900,color:GREEN}}>Walk {walk} min</div>}
              <div style={{width:3,height:34,background:'#d9d5e5',margin:'6px auto'}}/><div/><div/>
          ...
          <div style={{display:'flex',alignItems:'center',gap:22,padding:'28px 0',borderBottom:'1px solid #eeeaf7'}}>
            ...
            <div style={{flex:1}}><div style={{fontSize:27,fontWeight:900}}>Found data issues?</div><div style={{fontSize:18,color:'#77768a',lineHeight:1.35,marginTop:8}}>Tell us if the sign on the street does not match the app.</div></div>
            <button onClick={()=>setReported(true)} style={{border:'1px solid #e8e4f1',background:'#fff',borderRadius:8,padding:'12px 18px',fontSize:18,fontWeight:900,color:reported?GREEN:BLUE,cursor:'pointer'}}>{reported?'Sent':'Report'}</button>
          </div>
          ...
          <div style={{fontSize:13,color:'#77768a',lineHeight:1.4,marginTop:16,textAlign:'center'}}>{segment.dataNote||'Always check road signs before parking.'}</div>
        </div>
      </div>
