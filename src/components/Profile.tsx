...
export default function Profile() {
  ...
  return <section className="profile-screen"><header className="screen-header"><h1>Profile</h1><p>Verified businesses can complete their listing and post local offers, jobs, free meals, or community support.</p></header><OwnerBusinessProfile /><AdminBlueBadgePanel /><AdminPanel /><div className="privacy-card"><h2>Business research export</h2><p>Download the exact HiStreets business list with IDs, names, addresses and data-completeness flags. Use this only for legal public/manual research, not Google Maps stored data.</p><button onClick={downloadBusinessResearchList}><FileSpreadsheet size={18} /> Download business research CSV</button></div><div className="privacy-card"><h2>Privacy settings</h2><button onClick={downloadData}><Download size={18} /> Download my data</button><button className="danger" onClick={deleteAccount}><Trash2 size={18} /> Delete my account</button>{message && <p className="form-status">{message}</p>}<p><a href="/privacy.html">Privacy policy</a> · <a href="/terms.html">Terms</a></p></div></section>
}
