...
function Logo(){return <div style={{fontSize:30,fontWeight:900,letterSpacing:'-.8px',color:INK}}>Hi-Streets<span style={{color:BLUE}}>+</span><span style={{fontSize:18,marginLeft:8,color:'#77768a'}}>Newham business</span></div>}
...
function OfferPreview({offer,onPost,posting,posted,onEdit}){return <div className="chat-in" style={{alignSelf:'flex-start',width:'100%',background:'#fff',border:'1px solid #eeeaf7',borderRadius:18,padding:16,boxShadow:'0 8px 24px rgba(29,24,60,.08)'}}>
  <div style={{fontSize:13,fontWeight:900,color:'#77768a',marginBottom:8}}>Newham offer preview</div>
...
export default function BusinessPage(){
  ...
  useEffect(()=>{if(!messages.length)setMessages([{type:'bot',text:'Write your Newham offer in plain English. I will turn it into a clean map offer, then publish it for free after your business is verified.',id:1}])},[messages.length])
  ...
  async function saveProfile(){
    ...
    setSaving(false)
    if(result.success){setBusiness(result.business);setForm(formFromBusiness(result.business));setNotice(result.business.is_verified?'Newham business profile saved.':'Newham business profile submitted. It needs approval before offers appear on the public map.');setTab('chat')}
  ...
  return(
    <div style={{height:'100dvh',background:'#f7f6fc',display:'flex',flexDirection:'column',overflow:'hidden',color:INK}}>
      ...
      {!user&&<div style={{background:'#fff4df',borderBottom:'1px solid #f5e2b8',padding:'12px 18px',fontSize:14,fontWeight:800,color:INK,display:'flex',gap:10,alignItems:'center',justifyContent:'space-between'}}><span>Sign in to create a free Newham business profile and publish offers.</span><button onClick={()=>r.push('/login?redirect=/business')} className="solid-btn" style={{height:40,fontSize:14,padding:'0 14px'}}>Sign in</button></div>}
