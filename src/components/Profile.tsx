...
export default function Profile({ onPost }: Props) {
  ...
  if (!signedIn) return <section className="profile-screen"><div className="auth-card"><ShieldCheck size={34} /><h1>Business sign in</h1><p>Businesses sign in here to register, complete their listing, post offers/jobs/community help, and view job applications. Normal users do not need login to browse or apply.</p><input type="email" placeholder="you@example.com" value={email} onChange={e => setEmail(e.target.value)} /><button onClick={signIn}><LogIn size={18} /> Send magic link</button>{message && <p className="form-status">{message}</p>}<p className="tiny-links"><a href="/privacy.html">Privacy</a> · <a href="/terms.html">Terms</a></p></div></section>
