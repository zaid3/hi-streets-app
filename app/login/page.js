...
export default function LoginPage() {
  ...
  const [email,setEmail]=useState(''), [otp,setOtp]=useState(''), [step,setStep]=useState('email'), [loading,setLoading]=useState(false), [error,setError]=useState('')
  ...
  async function verifyOTP() {
    if(otp.length<6)return; setLoading(true); setError('')
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'#0a0a0a', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:24, fontFamily:'Arial,Helvetica,sans-serif' }}>
      ...
      <div style={{ marginBottom:44, zIndex:1, display:'inline-flex', flexDirection:'column', fontFamily:'Arial,Helvetica,sans-serif', lineHeight:1 }}>
      ...
      <div style={{ background:'rgba(255,255,255,.05)', backdropFilter:'blur(20px)', border:'1px solid rgba(255,255,255,.1)', borderRadius:20, padding:28, width:'100%', maxWidth:360, zIndex:1 }}>
        {step==='email'?(<>
          ...
          <p style={{ margin:'0 0 24px', fontSize:13, color:'rgba(255,255,255,.45)' }}>We will send a 6-digit code to your email</p>
          ...
          <button onClick={sendOTP} disabled={loading||!email} style={{ width:'100%', padding:16, background:loading||!email?'rgba(255,104,31,.35)':'linear-gradient(135deg,#ff681f,#FF8C00)', border:'none', borderRadius:12, color:'#fff', fontSize:16, fontWeight:700, cursor:loading||!email?'not-allowed':'pointer', boxShadow:'0 4px 20px rgba(255,104,31,.3)' }}>{loading?'Sending...':'Send Code'}</button>
          ...
          {error&&<p style={{ color:'#ff6b6b', fontSize:13, marginBottom:12 }}>{error}</p>}
          <button onClick={verifyOTP} disabled={loading||otp.length<6} style={{ width:'100%', padding:16, background:loading||otp.length<6?'rgba(255,104,31,.35)':'linear-gradient(135deg,#ff681f,#FF8C00)', border:'none', borderRadius:12, color:'#fff', fontSize:16, fontWeight:700, cursor:loading||otp.length<6?'not-allowed':'pointer', boxShadow:'0 4px 20px rgba(255,104,31,.3)', marginBottom:12 }}>{loading?'Verifying...':'Enter Hi-Streets'}</button>
