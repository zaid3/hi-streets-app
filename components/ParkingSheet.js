...
function nextLabel(seg){
  if(isCpz(seg))return seg.restriction||'Official CPZ zone. Check zone operating hours and bay signs before parking.'
...
function HoursTimeline({seg}){
  ...
  const restricted=seg.type==='resident'||seg.type==='permit'||seg.type==='loading'||isYellow(seg.type)||isCpz(seg)
