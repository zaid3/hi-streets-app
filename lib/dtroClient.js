...
function needsGzipFallback(error){
  return /Response size too large|dtros\/gzip/i.test(error?.message||'')
}
...
function authorisedHeaders(extra={}){
  const cfg=dtroConfig()
  return getDtroAccessToken().then(token=>{
    const headers={Authorization:`Bearer ${token}`,Accept:'application/json',...extra}
    if(cfg.appId)headers['App-Id']=cfg.appId
    return headers
  })
}
...
export async function dtroRequest(path,{method='GET',body,headers={}}={}){
  const cfg=dtroConfig()
  const requestHeaders=await authorisedHeaders(headers)
...
export async function dtroBinaryRequest(path){
  const cfg=dtroConfig()
  const res=await fetch(`${cfg.baseUrl}${path}`,{
    headers:await authorisedHeaders({'Accept':'application/gzip, application/octet-stream, application/json'}),
    cache:'no-store',
    signal:timeoutSignal(60000),
  })
  if(!res.ok){
    const text=await res.text().catch(()=>'')
    throw new Error(`D-TRO binary request ${path} failed (${res.status}): ${text.slice(0,300)}`)
  }
  const buffer=Buffer.from(await res.arrayBuffer())
  try{return parseMaybeJson(gunzipSync(buffer).toString('utf8'))}
  catch{
...
export async function getDtroById(id){
  ...
  try{return await dtroRequest(`/dtros/${encodeURIComponent(id)}`)}
  catch(error){
    if(needsGzipFallback(error))return dtroBinaryRequest(`/dtros/gzip/${encodeURIComponent(id)}`)
    throw error
  }
}
