...
export default function SideMenu({onClose,onNav,onSignOut,user}){
  ...
  return(
    <div style={{position:'fixed',inset:0,zIndex:3000}}>
      ...
      <div style={{position:'absolute',top:0,left:0,bottom:0,width:300,background:'rgba(8,8,8,.99)',backdropFilter:'blur(30px)',borderRight:'1px solid rgba(255,255,255,.08)',display:'flex',flexDirection:'column'}}>
        <div style={{padding:'24px 20px 20px',borderBottom:'1px solid rgba(255,255,255,.07)'}}>
          <div style={{display:'inline-flex',flexDirection:'column',fontFamily:'Arial,sans-serif',lineHeight:1,marginBottom:12}}>
        ...
        <div style={{flex:1,overflowY:'auto',padding:'8px 0'}}>
          {items.map(([ic,lb,ac])=>(
            <button key={ac} onClick={()=>{onNav(ac);onClose()}} style={{width:'100%',background:'none',border:'none',padding:'14px 20px',cursor:'pointer',display:'flex',alignItems:'center',gap:14,color:'rgba(255,255,255,.8)',fontSize:15,textAlign:'left'}}>
              <span style={{fontSize:20,width:26}}>{ic}</span>{lb}
            </button>
          ))}
        </div>
