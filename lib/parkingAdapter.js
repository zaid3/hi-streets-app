...
export async function getParkingData(bounds){
  const[curated,council,paidBays]=await Promise.all([fetchCuratedParking(bounds),fetchCouncilParking(bounds),fetchPaidBays(bounds)])
  return mergeById([...curated,...council,...paidBays])
}
...
async function fetchPaidBays(bounds){
  if(!bounds)return[]
  try{
    const q=new URLSearchParams({south:String(bounds.south),west:String(bounds.west),north:String(bounds.north),east:String(bounds.east)})
    const res=await fetch(`/api/parking/paid-bays?${q.toString()}`,{cache:'no-store'})
    if(!res.ok)return[]
    const json=await res.json()
    return json.items||[]
  }catch{return[]}
...
function priority(item){return item.source==='curated'||item.source==='dtro'?0:item.source==='council'?1:item.source==='paybyphone'?2:3}
